...
rawdata_file <- "../Rawdata/SM Run 3_3.csv"
...
#    - "B": background wells
#    - 0: wells to discard
metadata_file <- "../Rawdata/metadata run 3CSV.csv"
...
#Link the row to the medium (correct except for column 12, add the medium there)
metadata <- metadata %>%
  group_by(plate_row) %>%
  mutate(
    ...
    corrected_medium = ifelse(
      medium == "MM",
      ifelse(is.na(mm_medium), medium, mm_medium),
      medium
    )
  ) %>%
  ungroup() %>%
  select(-mm_medium)  # optional: clean up helper columns
...
plot_data_adjusted$col_bacteria <- paste(plot_data_adjusted$plate_col+10, plot_data_adjusted$bacteria)
...
if (basename == "SM Run 2_1" | basename == "SM Run 2_2" | basename == "SM Run 2_3"| basename == "SM Run 2_4"| basename == "SM Run 2_5") {
